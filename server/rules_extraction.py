...
def get_rules(node, parent_key, rules, grandparent_key="root"):
    ...
    NUMBER_PROPERTIES = [
        ...
        "value[flag]",
        "value[geneHeight]",
        "value[geneLabelFontSize]",
        "value[geneLabelOpacity]",
        "value[geneLabelStrokeThickness]",
        "value[opacity]",
        "value[size]",
        "value[strokeWidth]",
        "value[stroke]",
    ...
    adjusted_key = (
        f"{parent_key}[{grandparent_key}]"
        if parent_key in KEYS_WITH_GRANDPARENTS
        else parent_key
    )  # LHS
    ...
    for k in sorted(node.keys()):
        ...
        if isinstance(v, dict):
            ...
            if new_parent in ARRAY_PROPERTIES:
                ...
                "views",
                "dataTransforms",
            ]:  # Special handling for tracks, views, dataTransform
                for index, item in enumerate(v):
                    if isinstance(
                        item, dict
                    ):  # Ensure it's a dictionary before recursion
                        get_rules(
                            item, k[:-1], rules, parent_key
                        )  # Convert "tracks" -> "track" etc.
                    else:
                        rules.append(
                            k[:-1] + " -> " + '"' + str(item) + '"'
                        )  # Terminal rule
            ...
            if isinstance(
                v, bool
            ):  # Check boolean first, since True is treated as 1 in arithmetic operations
    ...
    with open(input_file, "r") as inputs:
        for line in inputs:
            try:
                ...
                rename_data_transform(
                    spec
                )  # Rename "dataTransform" to "dataTransforms"
                specs.append(spec)
